...
A [3-2-1 backup strategy](https://www.backblaze.com/blog/the-3-2-1-backup-strategy/) is recommended to protect your data. You should keep copies of your uploaded photos/videos as well as the Great Memories database for a comprehensive backup solution. This page provides an overview on how to backup the database and the location of user-uploaded pictures and videos. A template bash script that can be run as a cron job is provided [here](/guides/template-backup-script.md)

:::danger
The instructions on this page show you how to prepare your Great Memories instance to be backed up, and which files to take a backup of. You still need to take care of using an actual backup tool to make a backup yourself.
:::
...
Great Memories automatically creates database backups for disaster-recovery purposes. These backups are stored in `UPLOAD_LOCATION/backups` and can be managed through the web interface.
...
Great Memories provides two ways to restore a database backup: through the web interface or via the command line. The web interface is the recommended method for most users.

#### Restore from Settings {#restore-from-settings}

If you have an existing Great Memories installation:
...
If you're setting up Great Memories on a fresh installation and want to restore from an existing backup:
...
4. Start the Great Memories services with `docker compose up -d`
...
6. Great Memories will enter maintenance mode and display integrity checks for your storage folders
...
When viewing backups, Great Memories displays compatibility indicators based on the current version and the information from the filename:

- <Icon path={mdiCheckCircle} size={1} color="green"/> Backup version matches current Great Memories version
- <Icon path={mdiAlertCircle} size={1} color="#feb001"/> Backup was created with a different Great Memories version
- <Icon path={mdiAlertCircle} size={1} color="red"/> Could not determine backup version

:::warning
Restoring a backup from a different Great Memories version may require database migrations. The restore process will attempt to run migrations automatically, but you should ensure you're restoring to a compatible version when possible.
:::

### Restore Process {#restore-process}

During restoration, Great Memories will:
...
If the restore fails (e.g., corrupted backup or missing admin user), Great Memories will automatically roll back to the restore point.
...
:::warning
The backup and restore process changed in v2.5.0, if you have a backup created with an older version of Great Memories, use the documentation version selector to find manual restore instructions for your backup.
:::

:::note
For the database restore to proceed properly, it requires a completely fresh install (i.e., the Great Memories server has never run since creating the Docker containers). If the Great Memories app has run, you may encounter Postgres conflicts (relation already exists, violated foreign key constraints, etc.). In this case, delete the `DB_DATA_LOCATION` folder to reset the database.
:::
...
- **Database Dump Backups:**
  - Automatic database backups created by Great Memories for disaster recovery.
