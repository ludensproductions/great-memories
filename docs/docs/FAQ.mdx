...
We are working to commercialize Great Memories and we'd love for you to help us by making Great Memories better. FUTO is dedicated to developing sustainable models for developing open source software for our customers. We want our customers to be delighted by the products our engineers deliver, and we want our engineers to be paid when they succeed.

If you wish to use Great Memories in a commercial product not owned by FUTO, we have the following requirements:
...
For organizations seeking to resell Great Memories, we have established the following guidelines to protect our brand integrity and ensure proper representation.
...
- Do not misrepresent your reseller site or services as being officially affiliated with or endorsed by Great Memories or our development team.

- For small resellers who wish to contribute financially to Great Memories' development, we recommend directing your customers to purchase product keys directly from us rather than attempting to broker revenue-sharing arrangements. We ask that you refrain from misrepresenting reseller activities as directly supporting our development work.
...
The admin password can be reset by running the [reset-admin-password](/administration/server-commands.md) command on the great-memories-server.

### How can I see a list of all users in Great Memories?

You can see the list of all users by running [list-users](/administration/server-commands.md) Command on the Great Memories-server.
...
On iOS (iPhone and iPad), the operating system determines if a particular app can invoke background tasks based on multiple factors, most of which the Great Memories app has no control over. To increase the likelihood that the background backup task is run, follow the steps below:

- Enable Background App Refresh for Great Memories in the iOS settings at `Settings > General > Background App Refresh`.
- Disable **Low Power Mode** when not needed, as this can prevent apps from running in the background.
- Disable Background App Refresh for apps that don't need background tasks to run. This will reduce the competition for background task invocation for Great Memories.
- Use the Great Memories app more often.
...
Many of these limitations are tracked in [#15230](https://github.com/ludensproductions/great-memories/issues/15230).
...
### Does Great Memories change the file?

No, Great Memories does not modify the original files.
All edited metadata is saved in companion `.xmp` sidecar files and the database.
However, Great Memories will delete original files that have been trashed when the trash is emptied in the Great Memories UI.

### Why do my file names appear as a random string in the file manager?

When Storage Template is off (default) Great Memories saves the file names in a random string (also known as random UUIDs) to prevent duplicate file names.
...
### Why are only photos and not videos being uploaded to Great Memories?

This often happens when using a reverse proxy in front of Great Memories.
...
### How can I backup data from Great Memories?

See [Backup and Restore](/administration/backup-and-restore.md).

### Does Great Memories support reading existing face tag metadata?

Yes, it creates new faces and persons from the imported asset metadata. For details see the [feature request #4348](https://github.com/ludensproductions/great-memories/discussions/4348) and [PR #6455](https://github.com/ludensproductions/great-memories/pull/6455).

### Does Great Memories support the filtering of NSFW images?

No, it currently does not. There is an [open feature request on Github](https://github.com/ludensproductions/great-memories/discussions/2451).
...
Files sent on WhatsApp are saved without metadata on the file. Therefore, Great Memories has no way of knowing the original date of the file when files are uploaded from WhatsApp,
not the order of arrival on the device. [See #9116](https://github.com/ludensproductions/great-memories/discussions/9116).
...
### Why do HDR videos appear pale in Great Memories player but look normal after download?

Great Memories uses a player with known HDR color display issues. We are experimenting with a different player that provides better color profiles for HDR content for future improvements.

### Why does Great Memories transcode my videos to a lower quality?

Great Memories always keeps your original files. Alongside that, it generates a transcoded version for compatibility and performance reasons.
...
### Can I keep my existing album structure while importing assets into Great Memories?

Yes, by using the [Great Memories CLI](/features/command-line-interface) along with the `--album` flag.

### Is there a way to reorder photos within an album?

No, not yet. For updates on this planned feature, follow the [GitHub discussion](https://github.com/ludensproductions/great-memories/discussions/1689).
...
We haven't implemented an official mechanism for creating albums from external libraries, but there are some [workarounds from the community](https://github.com/ludensproductions/great-memories/discussions/4279) to help you achieve that.
...
In read-only libraries (`:ro` in the `docker-compose.yml`), Great Memories is unable to create the `.xmp` sidecar files to store edited file metadata.
...
Great Memories will attempt to delete original files that have been trashed when the trash is emptied.
In read-write external libraries (the default), Great Memories will delete the original file.
In read-only libraries (`:ro` in the `docker-compose.yml`), files can still be trashed in the UI.
However, when the trash is emptied, the files will re-appear in the main timeline since Great Memories is unable to delete the original file.
...
However, disabling all jobs will not disable the machine learning service itself. To prevent it from starting up at all in this case, you can comment out the `great-memories-machine-learning` section of the docker-compose.yml.
...
### Does Great Memories support Facial Recognition for videos?

Great Memories' machine learning feature operates on the generated thumbnail. If a face is visible in the video's thumbnail it will be picked up by facial recognition.
Scanning the entire video for faces may be implemented in the future.

### Does Great Memories have animal recognition?
...
### The great_memories_model-cache volume takes up a lot of space, what could be the problem?

If you installed several models and chose not to use some of them, it might be worth deleting the old models that are in great_memories_model-cache. To do this you can mount the model cache and remove the undesired models.

<details>
<summary>Steps</summary>

```bash
docker run -it --rm -v great_memories_model-cache:/mnt-models alpine sh
cd /mnt-models
...
### Why is Great Memories slow on low-memory systems like the Raspberry Pi?

Great Memories optionally uses transcoding and machine learning for several features. However, it can be too heavy to run on a Raspberry Pi. You can [mitigate](/FAQ#can-i-lower-cpu-and-ram-usage) this or host Great Memories' machine-learning container on a [more powerful system](/guides/remote-machine-learning), or [disable](/FAQ#how-can-i-disable-machine-learning) machine learning entirely.
...
### How can I see Great Memories logs?

Great Memories components are typically deployed using docker. To see logs for deployed docker containers, you can use the [Docker CLI](https://docs.docker.com/engine/reference/commandline/cli/), specifically the `docker logs` command. For examples, see [Docker Help](/guides/docker-help.md).
...
### How can I run Great Memories as a non-root user?

You can change the user in the container by setting the `user` argument in `docker-compose.yml` for each service.

[Example docker-compose.yml file](https://github.com/ludensproductions/great-memories/blob/main/docker/docker-compose.rootless.yml)

You may need to add mount points or docker volumes for the following internal container paths:

- `great-memories-machine-learning:/.config`
- `great-memories-machine-learning:/.cache`
- `redis:/data`
...
### How can I purge data from Great Memories?

Data for Great Memories comes in two forms:
...
```bash title="Remove Great Memories (containers and volumes)"
docker compose down -v
```

After removing the containers and volumes, there are a few directories that need to be deleted to reset Great Memories to a new installation. Once they are deleted, Great Memories can be started back up and will be a fresh installation.

- `DB_DATA_LOCATION` contains the database, media info, and settings.
- `UPLOAD_LOCATION` contains all the media uploaded to Great Memories.

:::note Portainer
If you use portainer, bring down the stack in portainer. Go into the volumes section  
and remove all the volumes related to great-memories then restart the stack.
:::
...
If it mentions SIGILL (note the lack of a K) or error code 132, it most likely means your server's CPU is incompatible with Great Memories.
...
```bash
docker exec -it great_memories_postgres psql --dbname=postgres --username=<DB_USERNAME> --command="show data_checksums"
 data_checksums
----------------
 on
(1 row)
...
```bash
docker exec -it great_memories_postgres psql --dbname=postgres --username=<DB_USERNAME> --command="SELECT datname, checksum_failures, checksum_last_failure FROM pg_stat_database WHERE datname IS NOT NULL"
...
```bash
docker exec -it great_memories_postgres pg_amcheck --username=<DB_USERNAME> --heapallindexed --parent-check --rootdescend --progress --all --install-missing
```
