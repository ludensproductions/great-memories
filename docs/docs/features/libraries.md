...
External libraries track assets stored in the filesystem outside of Great Memories. When the external library is scanned, Great Memories will load videos and photos from disk and create the corresponding assets. These assets will then be shown in the main timeline, and they will look and behave like any other asset, including viewing on the map, adding to albums, etc. Later, if a file is modified outside of Great Memories, you need to scan the library for the changes to show up.

If an external asset is deleted from disk, Great Memories will move it to trash on rescan. To restore the asset, you need to restore the original file. After 30 days the file will be removed from trash, and any changes to metadata within Great Memories will be lost.

:::caution

If you add metadata to an external asset in any way (i.e. add it to an album or edit the description), that metadata is only stored inside Great Memories and will not be persisted to the external asset file. If you move an asset to another location within the library all such metadata will be lost upon rescan. This is because the asset is considered a new asset after the move. This is a known issue and will be fixed in a future release.
...
Sometimes, an external library will not scan correctly. This can happen if Great Memories can't access the files. Here are some things to check:
...
If you encounter an `ENOSPC` error, you need to increase your file watcher limit. In sysctl, this key is called `fs.inotify.max_user_watches` and has a default value of 8192. Increase this number to a suitable value greater than the number of files you will be watching. Note that Great Memories has to watch all files in your import paths including any ignored files.
...
In rare cases, the library watcher can hang, preventing Great Memories from starting up. In this case, disable the library watcher in the configuration file. If the watcher is enabled from within Great Memories, the app must be started without the microservices. Disable the microservices in the docker compose file, start Great Memories, disable the library watcher in the admin settings, close Great Memories, re-enable the microservices, and then Great Memories can be started normally.
...
Let's show a concrete example where we add an existing gallery to Great Memories. Here, we have the following folders we want to add:

- `/home/user/old-pics`: a folder containing childhood photos.
- `/mnt/nas/christmas-trip`: photos from a christmas trip. The subfolder `/mnt/nas/christmas-trip/Raw` contains the raw files directly from the DSLR. We don't want to import the raw files to Great Memories
...
+     - /mnt/media/videos2:/mnt/media/videos2 # WARNING: Great Memories will be able to delete the files in this folder, as it does not end with :ro
...
These actions must be performed by the Great Memories administrator.
