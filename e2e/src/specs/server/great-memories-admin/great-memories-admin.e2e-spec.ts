import { greatMemoriesAdmin, utils } from 'src/utils';
...
describe(`great-memories-admin`, () => {
  ...
  describe('revoke-admin', () => {
    it('should revoke admin privileges from a user', async () => {
      const { child, promise } = greatMemoriesAdmin(['revoke-admin']);
  ...
  describe('grant-admin', () => {
    it('should grant admin privileges to a user', async () => {
      const { child, promise } = greatMemoriesAdmin(['grant-admin']);
  ...
  describe('list-users', () => {
    it('should list the admin user', async () => {
      const { stdout, exitCode } = await greatMemoriesAdmin(['list-users']).promise;
      expect(exitCode).toBe(0);
  ...
  describe('reset-admin-password', () => {
    it('should reset admin password', async () => {
      const { child, promise } = greatMemoriesAdmin(['reset-admin-password']);
