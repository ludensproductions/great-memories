...
describe(`great-memories login`, () => {
  beforeEach(async () => {
    await utils.resetDatabase();
  });

  it('should require a url', async () => {
    const { stderr, exitCode } = await greatMemoriesCli(['login']);
    expect(stderr).toBe("error: missing required argument 'url'");
    expect(exitCode).toBe(1);
  });

  it('should require a key', async () => {
    const { stderr, exitCode } = await greatMemoriesCli(['login', app]);
    expect(stderr).toBe("error: missing required argument 'key'");
    expect(exitCode).toBe(1);
  });

  it('should require a valid key', async () => {
    const { stderr, exitCode } = await greatMemoriesCli(['login', app, 'great-memories-is-so-cool']);
  ...
  it('should login and save auth.yml with 600', async () => {
    ...
    const { stdout, stderr, exitCode } = await greatMemoriesCli(['login', app, key.secret]);
    expect(stdout.split('\n')).toEqual([
      'Logging in to http://127.0.0.1:2285/api',
      'Logged in as admin@immich.cloud',
      'Wrote auth info to /tmp/great-memories/auth.yml',
    ]);
    expect(stderr).toBe('');
    expect(exitCode).toBe(0);

    const stats = await stat('/tmp/great-memories/auth.yml');
  ...
  it('should login without /api in the url', async () => {
    const admin = await utils.adminSetup();
    const key = await utils.createApiKey(admin.accessToken, [Permission.All]);
    const { stdout, stderr, exitCode } = await greatMemoriesCli(['login', app.replaceAll('/api', ''), key.secret]);
    expect(stdout.split('\n')).toEqual([
